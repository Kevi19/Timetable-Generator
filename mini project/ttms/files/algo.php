...
for ($i = 1; $i < 6; $i += 2) {
    ...
    echo $temp->dept;
    for ($j = 0; $j < 6; $j++) {
        $query = "UPDATE" . $database_name . "SET  course = '".$temp->dept."'  period1= '" . $subjectslots[$i][$j][0] . "<br>" . $aliasslots[$i][$j][0][0] . "',
...
period6='" . $subjectslots[$i][$j][5] . "<br>" . $aliasslots[$i][$j][5][0] . ", " . $aliasslots[$i][$j][5][1] . ", " . $aliasslots[$i][$j][5][2] . "'
 ...
 echo   $database_name . "     ". $temp->dept. "   ". $subjectslots[$i][$j][0] . "   " . $aliasslots[$i][$j][0][0] ." "
. $subjectslots[$i][$j][1] . "<br>" . $aliasslots[$i][$j][1][0] . " ". $subjectslots[$i][$j][2] . "   " . $aliasslots[$i][$j][2][0] . " ". $days[$j] ;

        $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"), $query);
        if($q)
        {
  echo "nksdkdnakd";
       echo "<script> alert('dkcskd')</script>";
    }
    else
...
for ($i = 0; $i < $teachers_count; $i++) {
    ...
    for ($j = 0; $j < 6; $j++) {
        $query = "UPDATE" . $database_name . " SET  period1= failed''" . $teachers[$i]->days[1][$j][0] . "<br>" . $teachers[$i]->classroom_names[1][$j][0] . "',
